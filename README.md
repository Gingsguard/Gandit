...
Gandit是对Bandit的二次开发。具体的检测流程如下：  
   ...
   - 然后进入简单预处理器，将这个叶子节点进行一些简单的处理.这里的简单预处理器相当于一个过滤器，配置文件位于core\check_list.py,这里需要将想要检查的函数名、方法名写入对应的列表中，Gandit只对在对应列表中的元素使用对应的高级预处理器进行处理与分析，若不在列表中的Call和Function，将不进行检查。这里加入简单预处理的目的是优化性能，防止一些如rsort()、base64_decode()、join()这样的不会造成安全隐患的Call进入检查器影响检测效率.
