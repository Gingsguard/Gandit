...
class BanditNodeVisitor(object):
    ...
    def visit_Call(self, node):
        ...
        def Find_root_FunctionDef_node(inputNode):
            '''
            Author:Ging'sGuard
            find root FunctionDef node
            eg: def test(a):
                    eval(a)
                Find_root_FunctionDef_node(eval_node): return test_node
            '''
            FunctionDef = ""
            while hasattr(inputNode,'parent'):
                if inputNode.__class__.__name__ == 'FunctionDef':
                    FunctionDef = inputNode
                inputNode = inputNode.parent
            return FunctionDef

        self.context['FunctionDef'] = Find_root_FunctionDef_node(node)
        ...
        self.context['qualname'] = qualname
        self.context['name'] = name
        if qualname in check_list.checkList['Call']:
            check = True
        else:
            check = False
        self.update_scores(self.tester.run_tests(self.context, 'Call', check))
